...
const Contact = () => {
  ...
  // ── Ref for the contact form section — used by "Email Us" smooth scroll ───
  ...
  //   2. Add your website — it will give you a Property ID and Widget ID
  //   3. Replace the two placeholder values below:
  //        TAWK_PROPERTY_ID  →  looks like:  67abc123def456789012abcd
  //        TAWK_WIDGET_ID    →  looks like:  1habcdefg
  ...
  useEffect(() => {
    ...
    // Tawk.to requires this global config object before the script loads
  ...
  // LIVE CHAT — opens the Tawk.to widget; falls back to WhatsApp if not set up yet
  const handleStartChat = () => {
    if (window.Tawk_API && typeof window.Tawk_API.maximize === "function") {
      // Tawk.to is loaded — pop open the chat bubble
      window.Tawk_API.maximize();
    } else {
      // Tawk.to not configured yet — send them to WhatsApp as a fallback
      // so the user is never left with a broken button
      window.open(
        `https://wa.me/254717575102?text=${encodeURIComponent(
          "Hello OptiLux! I would like to chat with your team."
        )}`,
        "_blank"
      );
    }
  ...
  // CALL US — tel: link
  // • Smartphone  → dials +254717575102 immediately
  // • Desktop     → opens Skype / Google Voice / any default softphone app
  //                 (if none installed, nothing happens — universal standard)
  const handleCall = () => {
    window.location.href = "tel:+254717575102";
  };

  // EMAIL US — smoothly scrolls down to the contact form on this same page
  const handleEmailUs = () => {
    if (formRef.current) {
  ...
  // WHATSAPP — official wa.me deep link (works on all devices)
  // • Smartphone  → opens WhatsApp app directly to the OptiLux chat
  // • Desktop     → opens WhatsApp Web (web.whatsapp.com) to the OptiLux chat
  // The pre-filled message makes it instantly easy for the customer to start
  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      "Hello OptiLux! 👓 I visited your website and would like to get in touch."
    );
    window.open(`https://wa.me/254717575102?text=${message}`, "_blank");
  };

  // ══════════════════════════════════════════════════════════════════════════
  // CHANNEL CARDS — each card now has its own dedicated handler
  // ══════════════════════════════════════════════════════════════════════════
  const channels = [
    {
      icon:    "💬",
      ...
      action:  "Start Chat",
      sub:     "Usually responds in under 2 minutes",
      onClick: handleStartChat,
    },
    {
      icon:    "📞",
      title:   "Call Us",
      desc:    "+254 717 575 102",
      action:  "Call Now",
    ...
    },
    {
      ...
      desc:    "+254 717 575 102",
      action:  "Message Us",
      sub:     "Quick replies, 7 days a week",
      onClick: handleWhatsApp,
    },
  ];

  // ══════════════════════════════════════════════════════════════════════════
  // FORM LOGIC — completely unchanged from previous version
  // ══════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className="contact-page">
      ...
      {/* CONTACT CHANNELS — every button is now fully functional */}
        ...
        <div className="contact-channels-inner">
          {channels.map((ch, i) => (
            <div key={i} className="contact-channel-card">
              ...
              <button className="contact-channel-btn" onClick={ch.onClick}>
                {ch.action}
              </button>
            </div>
          ))}
      ...
      <section className="contact-main">
        <div className="contact-main-inner">
          ...
          {/* FORM — ref attached so "Email Us" button scrolls here */}
              ...
              <div className="contact-form">
                ...
                <div className="contact-form-row">
                  <div className="contact-form-group">
                    ...
                    <input
                      type="text" name="name" placeholder="John Kamau"
                ...
                <div className="contact-form-row">
                  <div className="contact-form-group">
                    ...
                    <input
                      type="tel" name="phone" placeholder="+254 7XX XXX XXX"
          ...
          <div className="contact-faq-list">
            {faqs.map((faq, i) => (
              ...
              >
                ...
                {activeAccordion === i && (
                  <div className="contact-faq-answer"><p>{faq.a}</p></div>
                )}
              </div>
            ))}
          </div>
