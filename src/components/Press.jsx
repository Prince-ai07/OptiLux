...
const Press = () => {
  ...
  const openModal  = (assetTitle) => { setModal(assetTitle); setModalData({ name: "", email: "" }); setModalStatus(null); setModalMsg(""); };
  const closeModal = () => { setModal(null); setModalStatus(null); };

  const handleDownloadRequest = async () => {
    const { name, email } = modalData;
    if (!name.trim() || !email.trim() || !email.includes("@")) {
      setModalStatus("error"); setModalMsg("Please enter your full name and a valid email address."); return;
    }
    setModalStatus("loading");
    try {
      const res = await fetch("https://jeremiahprince.alwaysdata.net/api/contact", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), email: email.trim(), phone: "N/A — Press Kit Request", subject: "Press & Media",
          message: `Press Kit Download Request\n\nAsset requested: ${modal}\n\nPlease send the "${modal}" press kit assets to this email address.` })
      });
      const data = await res.json();
      if (data.success) { setModalStatus("success"); setModalMsg(`Got it! We will send the "${modal}" assets to ${email} within 24 hours.`); }
      else { setModalStatus("error"); setModalMsg("Something went wrong. Please email us directly at optilux60@gmail.com."); }
    } catch (err) {
      setModalStatus("error"); setModalMsg("Could not reach the server. Please email optilux60@gmail.com directly.");
    }
  };
    ...
    <div className="press-page">
        ...
        <div className="press-modal-overlay" onClick={closeModal}>
          <div className="press-modal" onClick={(e) => e.stopPropagation()}>
            <div className="press-modal-header">
              ...
              <div><h3>Request Press Asset</h3><p className="press-modal-asset-name">{modal}</p></div>
            ...
            {modalStatus === "success" ? (
              <div className="press-modal-success">
                ...
                <p>{modalMsg}</p>
                <button className="press-modal-done-btn" onClick={closeModal}>Done</button>
              </div>
            ) : (
              <div className="press-modal-body">
                <p className="press-modal-intro">Enter your details and our press team will email you the <strong> {modal}</strong> assets within 24 hours.</p>
                {modalStatus === "error" && <div className="press-modal-error">✕ {modalMsg}</div>}
                <div className="press-modal-field">
                  <label>Full Name *</label>
                  <input type="text" placeholder="Jane Ochieng" value={modalData.name} onChange={(e) => setModalData({ ...modalData, name: e.target.value })} disabled={modalStatus === "loading"} />
                </div>
                <div className="press-modal-field">
                  <label>Email Address *</label>
                  <input type="email" placeholder="jane@media.co.ke" value={modalData.email} onChange={(e) => setModalData({ ...modalData, email: e.target.value })} disabled={modalStatus === "loading"} />
                </div>
                <button className="press-modal-submit" onClick={handleDownloadRequest} disabled={modalStatus === "loading"}>
                  {modalStatus === "loading" ? "Sending Request..." : "Request Asset →"}
                </button>
                <p className="press-modal-note">By requesting assets you agree to use them only for editorial and media coverage of OptiLux.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="press-hero">
        <div className="press-hero-content">
          ...
          <p className="press-hero-sub">Media coverage, awards, press releases and brand resources. For media enquiries, contact our press team directly.</p>
          <a href="mailto:optilux60@gmail.com?subject=Press%20Enquiry%20—%20OptiLux&body=Hello%20OptiLux%20Press%20Team%2C%0A%0AI%20am%20reaching%20out%20regarding%3A" className="press-contact-btn">
            Contact Press Team →
          </a>
        </div>
      </section>
      ...
      <section className="press-facts">
        <div className="press-facts-inner">
          ...
          <div className="press-facts-grid">
            {facts.map((fact, i) => (
              <div key={i} className="press-fact-card"><h3>{fact.number}</h3><p>{fact.label}</p></div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="press-awards-list">
            {awards.map((award, i) => (
              <div key={i} className="press-award-item">
                ...
                <div className="press-award-info"><h4>{award.title}</h4><p>{award.body}</p></div>
            ...
            {coverage.map((item, i) => (
              <div key={i} className="press-coverage-card">
                <div className="press-coverage-header">
                  ...
                  <div><p className="press-outlet-name">{item.outlet}</p><p className="press-outlet-date">{item.date}</p></div>
      ...
      <section className="press-kit">
        <div className="press-kit-inner">
          ...
          <p className="press-kit-sub">Download official OptiLux brand assets, logos, product images, and company information for media use.</p>
          <div className="press-kit-cards">
            {kitAssets.map((kit, i) => (
              <div key={i} className="press-kit-card">
                ...
                <h4>{kit.title}</h4>
                <p>{kit.desc}</p>
                <button onClick={() => openModal(kit.title)}>Download</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="press-contact-inner">
          <div>
            ...
            <div className="press-contact-details">
              <div className="press-contact-item"><span>✉️</span><a href="mailto:optilux60@gmail.com?subject=Press%20Enquiry%20—%20OptiLux" style={{ color: "inherit", textDecoration: "none" }}>optilux60@gmail.com</a></div>
              <div className="press-contact-item"><span>📞</span><a href="tel:+254717575102" style={{ color: "inherit", textDecoration: "none" }}>+254 717 575 102</a></div>
              <div className="press-contact-item"><span>🕐</span><span>Mon – Fri, 9am – 5pm EAT</span></div>
            </div>
          </div>
          <div className="press-contact-cta">
            ...
            <button onClick={() => navigate("/contact")} className="about-cta-btn">Go to Contact Page →</button>
          </div>
        </div>
      </section>
